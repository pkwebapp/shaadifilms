...
export default function AlbumPage() {
  ...
  // Pinch-to-zoom: touchmove must be non-passive to call preventDefault
  useEffect(() => {
    const el = viewerRef.current;
    if (!el) return;
  ...
  const handleFindAlbum = (e: React.FormEvent) => {
    e.preventDefault();
    setIsFinding(true);
  ...
  return (
    ...
    >
      {/* Background video effect – muted, loop, low opacity */}
      ...
      <div className="relative z-10">
        ...
        <div className="container max-w-6xl">
          {/* Demo Album – same bg and border as Find Your Personal Album */}
          <div className="rounded-lg border bg-card text-card-foreground p-4 sm:p-6 md:p-8 shadow-sm">
            <div className="flex flex-col lg:flex-row gap-4 sm:gap-6 lg:gap-10">
              {/* Left: Album view (larger) – overflow hidden so zoom never overlaps right column */}
              <div className="flex-1 min-w-0 overflow-hidden">
                ...
                <div className="mt-4 sm:mt-6 flex flex-col items-start gap-3 sm:gap-4">
                  {/* Fixed-size wrapper so zoom never affects layout or overlaps Share Demo */}
                  <div
                    ref={viewerRef}
                  ...
                  >
                    {/* Exit fullscreen button – visible only when in fullscreen (e.g. on mobile) */}
                    {isFullscreen && (
                      <Button
                        type="button"
                        variant="secondary"
                    ...
                    >
                      ...
                      >
                        {/* Slide effect: current slides out, next slides in – next pic shown directly */}
                        <div
                          className="relative w-full h-full"
                          onTransitionEnd={handleTransitionEnd}
                        >
                          {/* Current page – slides out left (next) or right (prev) */}
                          <div
                          ...
                          {/* Next/prev page – slides in from right (next) or left (prev) */}
                          {flipTargetPage !== null && (
                            <div
                              className="absolute inset-0 flex items-center justify-center rounded-xl overflow-hidden bg-[#f5f2ee]"
                              style={{
